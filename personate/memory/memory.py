...
class Memory:
    ...
    async def retrieve_reply_chain(
        self, message: Union[discord.Message, InternalMessage], window_size: int = 15, max_characters: int = 800,
    ...
    async def retrieve_all_messages(
        self, msg: discord.Message, window_size: int = 15, max_characters: int = 800,
    ) -> List[InternalMessage]:
        ...
        for message in history[:window_size]:
            ...
            if sum([len(m.content) for m in past_messages]) > max_characters:
                break
        past_messages.reverse()
        converted_past_messages: List[InternalMessage] = [InternalMessage.from_discord_message(m) for m in past_messages]
