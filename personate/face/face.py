...
class Face:
    ...
    def __init__(self, bot: discord.Bot, avatar_url: str, username: str, loading_message: Optional[Union[List[str], str]] = None):
        self.bot = bot
        self.avatar_url = avatar_url
        self.username = username
        self.loading_message = loading_message
        self.webhooks: Dict[int, discord.Webhook] = {}
        logger.debug(f"Face created with avatar_url: {avatar_url} and username: {username}")
    ...
    async def get_webhook(self, channel_id: int) -> Optional[discord.Webhook]:
        ...
        if isinstance(channel, discord.TextChannel):
            ...
            if webhook:
                return webhook
        logger.debug(f"Was unable to find a webhook for channel: {channel_id}. This might be because the bot lacks the relevant permissions (Manage Webhooks), or for some other bizarre reason.")
        return None

    async def send_custom(self, channel: discord.TextChannel, content: str, avatar_url: str, username: str, **kwargs):
    ...
    async def send_loading(self, channel: discord.TextChannel):
        ...
        logger.debug(f"Sent loading message to channel: {channel.id} with content: {loading_message}")
        return await self.send_custom(channel, loading_message, self.avatar_url, self.username)

    async def send(self, channel: discord.TextChannel, content: str, **kwargs):
        logger.debug(f"Sent message to channel: {channel.id} with content: {content}")
        return await self.send_custom(channel, content, self.avatar_url, self.username, **kwargs)

    async def update(self, agent_message: InternalMessage, original_loading_message: discord.WebhookMessage):
        ...
        if agent_message.external_content and isinstance(agent_message.embeds[0], discord.Embed):
            await original_loading_message.edit(content=agent_message.external_content, embeds=agent_message.embeds, files=files)